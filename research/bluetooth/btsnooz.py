...
def decode_snooz(file, snooz):
    ...
    if version != 1 and version != 2:
        print('Unsupported btsnooz version: %s\n' % version)
        exit(1)
    # Oddly, the file header (9 bytes) is not compressed, but the rest is.
    decompressed = zlib.decompress(snooz[9:])
    file.write(b'btsnoop\x00\x00\x00\x00\x01\x00\x00\x03\xea')
    if version == 1:
        decode_snooz_v1(file, decompressed, last_timestamp_ms)
    elif version == 2:
        decode_snooz_v2(file, decompressed, last_timestamp_ms)


def decode_snooz_v1(file, decompressed, last_timestamp_ms):
    """
    ...
    while offset < len(decompressed):
        ...
        first_timestamp_ms += delta_time_ms
        offset += 7
        file.write(struct.pack('>II', length, length))
        file.write(struct.pack('>II', type_to_direction(type), 0))
        file.write(struct.pack(
            '>II', (first_timestamp_ms >> 32), (first_timestamp_ms & 0xFFFFFFFF)))
        file.write(type_to_hci(type))
        file.write(decompressed[offset: offset + length - 1])
        offset += length - 1


def decode_snooz_v2(file, decompressed, last_timestamp_ms):
    """
    ...
    while offset < len(decompressed):
        ...
        first_timestamp_ms += delta_time_ms
        offset += 9
        file.write(struct.pack('>II', packet_length, length))
        file.write(struct.pack('>II', type_to_direction(snooz_type), 0))
        file.write(struct.pack(
            '>II', (first_timestamp_ms >> 32), (first_timestamp_ms & 0xFFFFFFFF)))
        file.write(type_to_hci(snooz_type))
        file.write(decompressed[offset: offset + length - 1])
        offset += length - 1
    ...
    for line in iterator:
        if found:
            if line.find('--- END:BTSNOOP_LOG_SUMMARY') != -1:
                with open('./out.btsnoop', 'wb') as file:
                  decode_snooz(file, base64.standard_b64decode(base64_string))
                  sys.exit(0)
            base64_string += line.strip()
        if line.find('--- BEGIN:BTSNOOP_LOG_SUMMARY') != -1:
            found = True
    if not found:
        print('No btsnooz section found in bugreport.\n')
        sys.exit(1)
